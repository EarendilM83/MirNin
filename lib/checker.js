...
import { classifyWith } from './rules.js';
import { runLinkCheck } from './linkchecks.js';
...
// Classification lives in rules.js — checks record raw facts, the effective
// policy (global -> project -> category -> target) decides what they mean.
...
async function checkLocal(target, timeoutMs) {
  const started = performance.now();
  let url = target.url;
  let res = null;
  let redirects = 0;
...
let gpToken = null; // set from settings on every run; falls back to env
export const setGlobalpingToken = (t) => { gpToken = t || null; };

async function gpFetch(path, init) {
  const headers = { 'content-type': 'application/json', ...(init?.headers ?? {}) };
  const token = gpToken ?? process.env.GLOBALPING_TOKEN;
...
// ctx: { rules: effective policy, baselineFor(key) -> 7d median ms | null }
export async function runChecks(target, settings, ctx) {
  const t = Date.now();
  setGlobalpingToken(settings.globalpingToken);
  const provider = settings.provider ?? 'auto';

  // Non-http check types (redirect / ssl / domain / blocklist) run once per
  // target from a single vantage; they render as one GLOBAL-keyed tile.
  if (target.checkType && target.checkType !== 'http') {
    const entry = await runLinkCheck(target, settings, ctx.rules.timeoutMs);
    return [{ key: 'GLOBAL', loc: { country: 'GLOBAL', isp: null }, entry }];
  }
  ...
  const finish = (loc, o) => {
    ...
    out.push({
      ...
      entry: {
        t,
        status: classifyWith(o, ctx.rules, ctx.baselineFor(key)),
  ...
  const jobs = [];
  if (localLoc) jobs.push(checkLocal(target, ctx.rules.timeoutMs).then((o) => finish(localLoc, o)));
