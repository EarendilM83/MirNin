...
export class Store {
  ...
  #rollup(targetId, key, entry) {
    ...
    for (const h of Object.keys(this.rollups[targetId][key])) {
      if (Number(h) < minHour) delete this.rollups[targetId][key][h];
    }
  }
  ...
  // Called for every new entry; returns {opened: incident|null} so the caller
  // can trigger diagnostics on a fresh transition to down.
  trackIncident(targetId, loc, entry) {
    const open = this.openIncident(targetId, loc);
    if (entry.status === 'down') {
      if (open) { open.checks++; open.error = entry.error ?? open.error; this.dirty = true; return { opened: null }; }
      const incident = {
